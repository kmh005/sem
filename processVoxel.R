...
processVoxel <- function(v) {
    ...
    df <- data.frame(designmat$idnum, designmat$time, designmat$mcas,
                      designmat$panss_neg, designmat$age, Y)

    # Rename cols to sensible variables. E.g., "designmat.idnum" becomes "idnum"
    colnames(df) <- c("idnum", "time", "mcas", "panss_neg", "age", "Y")
    ...
    # # Regressions
    i ~ age + mcas + panss_neg
    s ~ age + mcas + panss_neg
    ...
    if (!is.null(fit)) {
        ...
        saveRDS(fit, file=paste0("fits/fit.", v, ".v1.rds"))
        
        graph_data <- prepare_graph(model = fit, layout = layout, angle=170) %>% edit_graph({ label = est }, element = "edges")

        fit_nodes<-nodes(graph_data)
        fit_edges<-edges(graph_data)

        saveRDS(fit_nodes, file=paste0("sem_graphs/nodes.", v, ".v1.rds"))
        saveRDS(fit_edges, file=paste0("sem_graphs/edges.", v, ".v1.rds"))

        g<-plot(graph_data) + ggtitle(v)

        ggsave(paste0("sem_graphs/sem.graph.", v, ".v1.pdf"), g, height=7, width=7)
        ...
        names(retvals) <- c("i~~i.est", "i~~s.est", "i~1.est", "i~age.est",
                            "i~mcas.est", "i~panss_neg.est",
                            "s~~s.est", "s~1.est", "s~age.est",
                            "s~mcas.est", "s~panss_neg.est",
                            "i~~i.pvalue", "i~~s.pvalue",
                            "i~1.pvalue", "i~age.pvalue",
                            "i~mcas.pvalue","i~panss_neg.pvalue",
                            "s~~s.pvalue", "s~1.pvalue", "s~age.pvalue",
                            "s~mcas.pvalue","s~panss_neg.pvalue",
                            "i~~i.est_std", "i~~s.est_std",
                            "i~1.est_std", "i~age.est_std",
                            "i~mcas.est_std","i~panss_neg.est_std",
                            "s~~s.est_std", "s~1.est_std", "s~age.est_std",
                            "s~mcas.est_std","s~panss_neg.est_std",
                            "i~~i.pvalue_std",
                            "i~~s.pvalue_std", "i~1.pvalue_std",
                            "i~age.pvalue_std",
                            "i~mcas.pvalue_std","i~panss_neg.pvalue_std",
                            "s~~s.pvalue_std", "s~1.pvalue_std",
                            "s~age.pvalue_std",
                            "s~mcas.pvalue_std","s~panss_neg.pvalue_std",
                            "chisq", "df", "pvalue", "rmsea", "rmsea.ci.lower",
                            "rmsea.ci.upper", "cfi", "tli", "srmr")
    }
